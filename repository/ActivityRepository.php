...
class ActivityRepository { 
   ...
   public static function getActivity(User $user = NULL, $start_date, $end_date) { 

     $results=[];

     $connexion = Database::dbConnection();
     $query = (Database::isSqlsrv() ? "SELECT * FROM [activity] WHERE [user_id]=:id AND [timestamp_creation]>=:start_create AND [timestamp_creation]<=:end_date" : "SELECT * FROM activity WHERE `user_id`=:id AND timestamp_creation>=:start_create AND timestamp_creation<=:end_date");
